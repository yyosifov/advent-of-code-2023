...
function intersectRanges(range1, range2, absDiff) {
    // Identify the intersection points
    const startIntersection = Math.max(range1[0], range2[0]);
    const endIntersection = Math.min(range1[1], range2[1]);

    // Check if there is a valid intersection
    if (startIntersection <= endIntersection) {
        // Generate slices based on the intersection
        const slices = [];
        if (startIntersection > range1[0]) {
            slices.push([range1[0], startIntersection]);
        }
        slices.push([startIntersection + absDiff, endIntersection + absDiff]);
        if (endIntersection < range1[1]) {
            slices.push([endIntersection, range1[1]]);
        }
        return slices;
    } else {
        // No intersection
...
function rec(ranges) {
    ...
    for (const m of maps) {
        ...
        for (const r of newRanges) {
            ...
            for (const [x, y, z] of m) {
                const crange = [y, y + z - 1];
                const absDiff = x - y;

                const inter = intersectRanges(r, crange, absDiff);
                if (inter.length) {
                    hasIntersect = true;

                    console.log(`intersection ${inter}, r: ${r[0]},${r[1]} for ${x}, ${y}, ${z}, absDiff: ${absDiff}, range: ${y}, ${y + z - 1}`);
                    currentRanges = [...currentRanges, ...inter];
                    break;
                }
            }
...
for (let i = 0; i < seeds.length; i += 2) {
    ...
    const len = seeds[i + 1];
    let to = from + len - 1
    const all = rec([[from, to ]]);
    console.log(`all = ${all}`);
    const cres = _.min(_.flatten(all));
    if (min == -1) min = cres;
    if (min > cres && cres != 0) min = cres;
    console.log(cres);
